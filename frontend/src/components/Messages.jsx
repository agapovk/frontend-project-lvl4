...
const Messages = () => {
	...
	const handleSubmit = (e) => {
		...
		socket.emit(
			'newMessage',
			{ body: message, channelId: chat.currentChannelId, username: user.username },
			(response) => {
	...
	const filteredMessages = (id) => {
		return chat.messages ? chat.messages.filter((mes) => mes.channelId === id) : [];
	};
	...
	return (
		<>
			...
			<div className='mb-3 small text-muted'>
				...
				{t('home.mesCount', { count: filteredMessages(chat.currentChannelId).length })}
			</div>
			<div
				id='messages'
