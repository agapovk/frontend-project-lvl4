...
const Home = () => {
	const { user, setShowAddModal, setShowDeleteModal, setShowRenameModal, setEditChannel } =
				...
				<div id='main' className='d-flex'>
					<aside className='col-4 col-lg-3 ms-4 my-4 bg-white p-3 border rounded-start d-flex flex-column'>
						<div className='d-flex justify-content-between align-items-center mb-3'>
							...
							<Button className='px-2 py-0' onClick={() => setShowAddModal(true)}>
								+
							</Button>
						</div>
						<ul className='list-unstyled flex-grow-1 overflow-auto'>
							{chat.channels &&
								chat.channels.map((c) => (
									<li key={c.id} className='d-flex justify-content-center align-items-center'>
										<Button
											id='btnHover'
											variant='secondary'
											...
											<span className={c.id === chat.currentChannelId ? 'fw-bold text-black' : ''}>
												{`# ${c.name}`}
											</span>
										</Button>
										{c.removable && (
