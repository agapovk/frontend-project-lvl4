...
const Signup = () => {
	...
	const onSubmit = async (values, actions) => {
		try {
			...
			console.error(error);
			setSignupError(error.response.status === 409 ? t('signup.error') : error.response.statusText);
		}
		...
		<div className='container-fluid  bg-light'>
			<div id='login' className='d-flex flex-column justify-content-center align-items-center'>
				<form
					...
					autoComplete='off'>
					<h1 className='text-dark text-center mb-3 fs-4'>{t('signup.header')}</h1>
					<div className='form-floating mb-3'>
						...
						{errors.name && touched.name && <p className={errorClassNames}>{errors.name}</p>}
					</div>
					<div className='form-floating mb-3'>
						<input
							id='password'
